    ...
    match File::open(&path) {
      Ok(mut image) => {
        ...
        for y in range(0, image_height) {
            ...
            match image.read_exact(3) {
              Ok(mut pixel_data) => {
                ...
                match pixel_data.pop() {
                  Some(red) => {image_data_bytes.push(red)},
                  None  => {fail!("Error getting red component for BMP pixel")}
                }
                match pixel_data.pop() {
                  Some(green) => {image_data_bytes.push(green)},
                  None  => {fail!("Error getting green component for BMP pixel")}
                }
                match pixel_data.pop() {
                  Some(blue) => {image_data_bytes.push(blue)},
          ...
          // Without this it flips over when converting to ppm
          // for byte in buffer.iter() {
          //   image_data_bytes.unshift(byte.clone());
          // }
...
fn main() {
  ...
  if args.len() < 2 {
    ...
    // PPM  --> PPM       Works
    // PPM  --> BMP       Works
    // BMP  --> PPM       Reading BMP results in scanlines being reversed --> image upside-down
    // BMP  --> BMP       Works
